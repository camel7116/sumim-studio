...
/* =========================================================================
   시안 ④ — 그래픽 장치 5종 (2026-08-31 · 미디어팔레트 방향)
   -------------------------------------------------------------------------
   레퍼런스(media-palette.co.kr)는 스밈과 **같은 팔레트**(보라+라임+검정/흰)인데도
   단조롭지 않다. 색을 배경이 아니라 **그래픽 장치**로 쓰기 때문이다 —
   ① 브랜드 도형 3종이 섹션마다 반복 ② 도형+단어 마퀴 띠 ③ 거대 영문 섹션 타이포
   ④ 라임 pill 배지 행 리스트 ⑤ 스크롤 텍스트 하이라이트.

   아래 스위치는 **전부 끄면 2026-08-31 이전 화면으로 그대로 돌아온다.**
   (④ 서비스 행 리스트만 `service-system.tsx` 상단 `DESKTOP_CARDS` 가 쥔다 —
    카드 배치 스위치가 원래 그 파일에 있어서 한 곳에 모았다)

   🚨 색은 전부 기존 토큰(`--color-cta`·`--color-canvas`·`--color-ink`)이거나
      **중첩 `data-band`** 다. hex 하드코딩은 없다(카카오 옐로만 전례 예외).
   🚨 §7 반려 목록 준수 — 도형은 **콘텐츠 요소로만** 쓴다. 배경 장식(파티클·격자·
      물감 번짐)으로 깔지 않는다("배경 장식 전면 off" 원칙).
   ========================================================================= */

/**
 * 🔀 **스위치 ④-① `brandGlyphs`** — 브랜드 도형 3종(파도·애스터리스크·도넛) 노출.
 *
 * - `true` : 섹션 헤더 옆 · 서비스 행 리스트 우측 끝 · Final CTA 헤딩 옆에
 *   작은 도형이 하나씩 붙는다(섹션마다 다른 도형이 순환한다).
 * - **`false`(현재)** : 도형이 **한 개도 렌더되지 않는다**(2026-08-31 이전 화면).
 *
 * 구현은 `components/ui/brand-glyphs.tsx` — 인라인 SVG 3종, 라이브러리 없음.
 * 🚨 2026-09-01 사용자 반려 "아이콘들도 대충 만든 느낌" → off. 거대 헤딩·서비스 행만 유지.
 */
export const brandGlyphs: boolean = false;

/**
 * 🔀 **스위치 ④-② `graphicTicker`** — Work 흰 밴드 **최상단 안쪽**의 그래픽 마퀴 띠.
 *
 * 단어(영문 대문자, 굵게) + 도형이 번갈아 흐르는 전폭 롤링 띠 한 줄이다.
 * - `true` / **`false`(현재)** : 띠가 통째로 빠진다.
 * 🚨 2026-09-01 사용자 반려 "너무 따라가려하는 느낌" → off (미디어팔레트 이식 티가 남).
 *
 * 🚨 **밴드 리듬은 불변**이다 — 새 섹션을 만들지 않고 Work(흰) 섹션 **안쪽 맨 위**에
 *    넣었다. Hero(검정) → Work(흰) 순서가 그대로다.
 * 🚨 구동은 기존 마키 훅(`components/ui/marquee.tsx` 의 `useMarquee`)을 그대로 쓴다 —
 *    화면 밖 정지 · `prefers-reduced-motion` 정지 · 드래그가 전부 따라온다.
 */
export const graphicTicker: boolean = false;

/**
 * 마퀴 띠에 흐르는 단어 — `services.ts` 의 `serviceAxes` 영문명과 **같은 결**이다
 * (Brand Strategy · Web Experience · Search Foundation · Content Growth).
 * 🚨 새 카피가 아니라 이미 화면에 있는 네 축을 한 단어로 줄인 것이다.
 * ⚠️ 단어를 더하거나 빼면 트랙 길이가 변한다 — 속도는 `px/s` 고정(아래 컴포넌트 상수)이라
 *    자동으로 맞춰지지만, 띠에 몇 단어가 보이는지는 달라진다.
 */
export const graphicTickerWords: readonly string[] = [
  "BRAND",
  "WEB",
  "SEO",
  "CONTENT",
...
/**
 * 🔀 **스위치 ④-③ `giantHeadings`** — 섹션 헤더 위 **거대 영문 타이포** 한 줄.
 *
 * - **`true`(현재 · 기본)** : 주요 섹션 헤더 위에 clamp(44px→90px) 영문 라벨이 서고,
 *   **그 섹션의 eyebrow(액센트 틱 + 12.5px 라벨)는 숨는다** — 같은 말이 두 번 나기 때문이다.
 * - `false` : 거대 타이포가 사라지고 **eyebrow 가 그대로 복귀**한다(2026-08-31 이전 화면).
 *
 * 색은 밴드 토큰(`--color-ink`)이라 밝은 밴드에서는 잉크 · 검정 밴드에서는 흰색이다.
 */
export const giantHeadings: boolean = true;

/**
 * 거대 영문 라벨 — **문자열은 여기 한 곳에서만** 관리한다.
 * 🚨 새 카피 0줄: 전부 각 섹션이 이미 쓰고 있던 eyebrow 문자열이거나 그 축약이다.
 */
/*
  ⚠️ **라벨은 짧게** — 헤더 블록이 `max-w-[720px]` 라 86~90px 글자로 13자를 넘으면 두 줄로 접힌다
     (실측: "SELECTED WORK" 는 720px 상자에서 **2줄 158px**). 그래서 Work 는 네비 메뉴와 같은
     **"WORK"** 로 줄였다. 새 라벨을 넣을 때도 한 줄에 들어가는지 먼저 재 볼 것.
*/
export const giantSectionLabels = {
  work: "WORK",
  services: "SERVICES",
  reviews: "REVIEWS",
  qna: "Q&A",
...
/**
 * 🔀 **스위치 ④-⑤ `bridgeTextHighlight`** — 브릿지 밴드 헤딩의 스크롤 하이라이트.
 *
 * "말로 설명하지 않습니다 / 만든 것으로 보여드립니다" 가 스크롤 진행에 따라
 * **흰 25% → 흰색**으로 왼쪽부터 차오른다.
 * - **`true`(현재 · 기본)** / `false` : 처음부터 끝까지 흰색(2026-08-31 이전 화면).
 *
 * 🚨 **히어로 워드마크에는 손대지 않는다** — 글자 일렁임·이동은 §7 반려 목록이다.
 *    이 효과는 브릿지 헤딩 하나뿐이고, 중간에 멈춰도 "여기까지 찼다"로 읽히는
 *    형태라 반려된 "스크롤 위치에 묶인 모핑"(반쯤 변한 채 서 있는 구조)과 다르다.
 * 🚨 구현은 **CSS 전용 scroll-driven animation**(`animation-timeline: view()`)이고
 *    `@supports` 로 감싸 미지원 브라우저는 **정적 흰색** 그대로다(JS 0줄).
 */
export const bridgeTextHighlight: boolean = true;
